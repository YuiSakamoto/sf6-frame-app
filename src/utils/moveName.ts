...
/** 現在の言語に合ったコンボ補正値を返す */
export function getComboScaling(move: Move, language: string): string | undefined {
  return isJa(language) ? move.comboScaling : (move.comboScalingEn ?? move.comboScaling);
}

/** 現在の言語に合った属性を返す */
export function getProperties(move: Move, language: string): string | undefined {
  return isJa(language) ? move.properties : (move.propertiesEn ?? move.properties);
}
